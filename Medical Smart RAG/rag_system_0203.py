...
class MedicalWebUI:
    ...
    def _adapter_ingest(self, file_obj):
        if file_obj is None: return "⚠️ Choose the file."
        if self.system.ingest_file(file_obj.name):
            return f"✅ Document analysis complete: {os.path.basename(file_obj.name)}"
        return "❌ Document analysis failure"
        ...
        with gr.Blocks(theme=self.theme, title="Medical AI Assistant") as demo:
            ...
            with gr.Row():
                ...
                with gr.Column(scale=7):
                    ...
                    gr.Markdown("*⚠️ Disclaimer: For an accurate diagnosis, please consult a physician.*")
